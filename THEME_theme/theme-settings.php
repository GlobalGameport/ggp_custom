 ...
 */
function THEME_theme_form_system_theme_settings_alter(&$form, &$form_state, $form_id = NULL)  {
  ...
  $form['#submit'][] = 'THEME_theme_settings_submit';
  $form['at']['background']['#element_validate'][] = 'THEME_theme_settings_submit';
  return $form;
}
/**
 * Save settings data.
*/
function THEME_theme_settings_submit($form, &$form_state) {
  $settings = array();
  ...
  if ($file = file_save_upload('bg_image')) {
    ...
    if ($image = _THEME_theme_save_image($file)) {
...
function _THEME_theme_save_image($file, $bg_folder = 'public://backgrounds/', $bg_thumb_folder = 'public://backgrounds/thumb/') {
